<template>
  <div class="container mx-auto px-4 py-8">
    <div class="max-w-7xl mx-auto">
      <!-- 頁面標題 -->
      <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">管理所有活動</h1>
        <p class="text-gray-600 mt-2">管理員可以查看、編輯和刪除所有活動</p>
      </div>
      ...
      <div v-else>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <Card class="bg-green-50">
            <template #content>
              <div class="text-center">
                <div class="text-2xl font-bold text-green-600">{{ totalAttendees }}</div>
                <div class="text-sm text-gray-600">總參與人數</div>
              </div>
            </template>
          </Card>
          <Card class="bg-purple-50">
            <template #content>
              <div class="text-center">
                <div class="text-2xl font-bold text-purple-600">{{ upcomingEvents }}</div>
          ...
          <Card class="bg-orange-50">
            <template #content>
              <div class="text-center">
                <div class="text-2xl font-bold text-orange-600">{{ pastEvents }}</div>
        ...
        <Card>
          ...
          <template #content>
            <div class="overflow-x-auto">
              <table class="w-full">
                <thead class="bg-gray-50">
                  <tr>
                    ...
                    <th class="px-4 py-3 text-left text-sm font-semibold text-gray-700">主辦者</th>
              ...
              <!-- 空狀態 -->
              <div v-if="eventStore.events.length === 0" class="text-center py-12 text-gray-500">
...
const totalAttendees = computed(() => {
  return eventStore.events.reduce((total, event) => {
    return total + (event.attendees?.length || 0)
  }, 0)
})

const upcomingEvents = computed(() => {
  return eventStore.events.filter((event) => isUpcoming(event.date)).length
})
